/**
 * Publish (activate) SCRUMMASTER_LESZEK_2026_HU Day 02 lesson (backup-first).
 *
 * Usage:
 * - Dry-run:
 *   npx tsx --env-file=.env.local scripts/publish-scrummaster-leszek-2026-hu-day-02.ts
 * - Apply (DB write + backup):
 *   npx tsx --env-file=.env.local scripts/publish-scrummaster-leszek-2026-hu-day-02.ts --apply
 */
...
async function main() {
  ...
  const backupFile = join(backupDir, `publish-day-02__${stamp}.json`);
