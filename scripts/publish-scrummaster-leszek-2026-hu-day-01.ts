/**
 * Publish (activate) SCRUMMASTER_LESZEK_2026_HU + Day 01 lesson (backup-first).
 *
 * Why:
 * - The seed creates the course + lessons as inactive drafts by default.
 * - The student-facing UI typically only shows active courses/lessons.
 *
 * Usage:
 * - Dry-run:
 *   npx tsx --env-file=.env.local scripts/publish-scrummaster-leszek-2026-hu-day-01.ts
 * - Apply (DB write + backup):
 *   npx tsx --env-file=.env.local scripts/publish-scrummaster-leszek-2026-hu-day-01.ts --apply
 */
...
async function main() {
  ...
  const backupFile = join(backupDir, `publish-day-01__${stamp}.json`);
  ...
  const backupPayload = {
    ...
    course: {
      ...
      certificationEnabled: Boolean((course as any)?.certification?.enabled ?? false),
      certificationPriceMoney: (course as any)?.certification?.priceMoney ?? null,
      certificationPricePoints: (course as any)?.certification?.pricePoints ?? null,
    },
    lesson: {
  ...
  if (!APPLY) {
    ...
    console.log(`- would set course.isActive=true and lesson.isActive=true`);
  ...
  const courseUpdate = await Course.updateOne({ courseId: COURSE_ID }, { $set: { isActive: true } });
  const lessonUpdate = await Lesson.updateOne({ lessonId: LESSON_ID }, { $set: { isActive: true } });

  console.log('✅ Published Day 01');
  console.log(`- courseId: ${COURSE_ID}`);
  console.log(`- course matched: ${courseUpdate.matchedCount}, modified: ${courseUpdate.modifiedCount}`);
  console.log(`- lessonId: ${LESSON_ID}`);
