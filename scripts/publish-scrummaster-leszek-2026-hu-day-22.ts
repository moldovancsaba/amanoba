/**
 * Publish (activate) SCRUMMASTER_LESZEK_2026_HU Day 22 lesson (backup-first).
 */
...
async function main() {
  ...
  const backupFile = join(backupDir, `publish-day-22__${stamp}.json`);

  const backupPayload = {
    generatedAt: new Date().toISOString(),
    courseId: COURSE_ID,
    lessonId: LESSON_ID,
    course: { courseId: String((course as any).courseId ?? ''), isActive: Boolean((course as any).isActive), requiresPremium: Boolean((course as any).requiresPremium) },
    lesson: { lessonId: String((lesson as any).lessonId ?? ''), isActive: Boolean((lesson as any).isActive), dayNumber: Number((lesson as any).dayNumber ?? -1), title: String((lesson as any).title ?? '') },
  };
