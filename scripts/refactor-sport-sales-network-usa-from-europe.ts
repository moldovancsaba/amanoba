/**
 * Refactor USA course using Europe recreated as the base format.
 *
 * What:
 * - Uses Europe recreated package as the structural template (lesson format, sections)
 * - Swaps course metadata to USA
 * - Replaces bibliography + read more with USA-specific mapping
 * - Rebuilds questions to USA-specific set (7 per lesson)
 *
 * Output:
...
const EUROPE_BASE = 'docs/course/SPORT_SALES_NETWORK_EUROPE_2026_EN_export_2026-02-06_RECREATED.json';
const USA_CANONICAL = 'docs/canonical/SPORT_SALES_NETWORK_USA_2026/SPORT_SALES_NETWORK_USA_2026.canonical.json';
...
function buildGenericQuestions(dayNumber: number, lessonTitle: string, baseTags: string[]): LessonQuizQuestion[] {
  ...
  const contextByDay: Record<number, { who: string; offer: string; buyer: string; constraint: string }> = {
    1: { who: 'a sales lead at a sports performance SaaS vendor', offer: 'a subscription platform for elite team performance tracking', buyer: 'a US collegiate athletics department', constraint: 'budget is split across performance, medical, and IT' },
    2: { who: 'a commercial manager at a venue technology company', offer: 'a ticketing + access control bundle', buyer: 'a multi-venue operator across US states', constraint: 'implementation must align with event calendars' },
    3: { who: 'a sales lead at a sports performance SaaS vendor', offer: 'a subscription platform for elite team performance tracking', buyer: 'a national federation with multiple departments', constraint: 'budget is split across performance, medical, and IT' },
    4: { who: 'a commercial manager at a venue technology company', offer: 'a ticketing + access control bundle', buyer: 'a multi-venue operator', constraint: 'implementation must align with event calendars' },
    5: { who: 'a founder selling a sponsorship activation analytics product', offer: 'measurement dashboards for brand activations', buyer: 'a sports rights holder and its top sponsor', constraint: 'ROI must be defensible to finance within 30 days' },
    6: { who: 'a partner manager at a fitness equipment supplier', offer: 'a financing-backed equipment package', buyer: 'a chain of boutique gyms', constraint: 'deal requires sign-off from finance, legal, and operations' },
    7: { who: 'a growth lead at a sport education platform', offer: 'a B2B course licensing model', buyer: 'clubs and academies across the USA', constraint: 'lead volume is high but qualified deals are scarce' },
    8: { who: 'a field sales manager at a venue services provider', offer: 'match-day operations software', buyer: 'stadium operators and event promoters', constraint: 'decision windows are tied to fixed event dates' },
    9: { who: 'a regional sales director', offer: 'a performance analytics suite', buyer: 'clubs across multiple US regions', constraint: 'coverage is limited; reps must prioritize accounts' },
    10: { who: 'a sales ops analyst', offer: 'a service + software package for clubs', buyer: 'mid-market club groups', constraint: 'gross margin varies heavily by deployment model' },
    11: { who: 'a revenue operations manager', offer: 'a three-motion GTM (direct, partner, procurement)', buyer: 'mixed segments across the USA', constraint: 'pipeline reporting is inconsistent across teams' },
    12: { who: 'an enterprise account executive', offer: 'a multi-year platform contract', buyer: 'a federation + its commercial arm', constraint: 'multiple stakeholders can veto late' },
    13: { who: 'a sales rep', offer: 'a sports facility booking platform', buyer: 'a university-owned facility operator', constraint: 'urgency is unclear and timelines drift' },
    14: { who: 'a solutions lead', offer: 'a pilot + rollout proposal for a venue network', buyer: 'a multi-venue operator', constraint: 'risk control and rollout governance are key' },
    15: { who: 'an account executive', offer: 'a negotiation for a 12-month contract', buyer: 'a procurement team', constraint: 'discount pressure is high near quarter end' },
    16: { who: 'a channel director', offer: 'a partner-led sales motion', buyer: 'resellers and consultancies', constraint: 'partner conflict and overlap with direct sales' },
    17: { who: 'a partner enablement lead', offer: 'onboarding for new partners', buyer: 'fitness and sport integrators', constraint: 'partners have low product knowledge initially' },
    18: { who: 'a channel ops manager', offer: 'deal registration + incentives', buyer: 'referral and reseller partners', constraint: 'disputes about deal ownership are increasing' },
    19: { who: 'a partner marketer', offer: 'co-marketing and events', buyer: 'strategic partners', constraint: 'lead quality from events is inconsistent' },
    20: { who: 'a sales enablement manager', offer: 'playbooks and tools', buyer: 'direct reps and partners', constraint: 'messaging and demos vary widely by rep' },
    22: { who: 'a compliance lead', offer: 'supplier readiness for US public bids', buyer: 'public contracting authorities', constraint: 'missing mandatory forms causes immediate exclusion' },
    23: { who: 'a sales ops lead', offer: 'tender discovery and monitoring', buyer: 'public sector sport and recreation buyers', constraint: 'the team misses notices because monitoring is ad hoc' },
    24: { who: 'a state sales manager', offer: 'state and local procurement opportunities', buyer: 'state agencies and public bodies', constraint: 'entry points differ from federal workflows' },
    25: { who: 'an international expansion lead', offer: 'public procurement entry points across regions', buyer: 'US federal and MENA government buyers', constraint: 'process differences create execution risk' },
    26: { who: 'a RevOps lead', offer: 'CRM discipline and data standards', buyer: 'internal sales and partner teams', constraint: 'pipeline data is unreliable for forecasting' },
    27: { who: 'a sales leader', offer: 'KPI system aligned to motions', buyer: 'direct + partner teams', constraint: 'teams optimize the wrong metrics' },
    28: { who: 'a compensation designer', offer: 'incentives across direct and channel', buyer: 'sales reps and partners', constraint: 'comp plans drive channel conflict and sandbagging' },
    29: { who: 'an enablement lead', offer: 'scripts, decks, demos, objection handling', buyer: 'direct reps and partners', constraint: 'objections recur and reps improvise weak answers' },
    30: { who: 'a GTM operator', offer: 'a 90-day rollout plan and cadence', buyer: 'the internal revenue org', constraint: 'too many initiatives launch without owners' },
  };
...
function applyReplacements(content: string, replacements: Array<[RegExp, string]>) {
  let updated = content;
...
function updateLessonHeaderTitle(content: string, newTitle: string) {
  return content.replace(/^# Lesson (\d+): .+$/m, `# Lesson $1: ${newTitle}`);
}
...
function replaceLine(content: string, re: RegExp, next: string) {
  return content.replace(re, next);
}

function replaceBlock(content: string, startHeader: string, endHeader: string, replacement: string) {
  return replaceBetween(content, startHeader, [endHeader, '\n# ', '\n## '], replacement);
}

function updateContentWithCanonical(content: string, canonical: CanonicalLesson) {
  let updated = content;

  const oneLiner = canonical.objective.replace(/\.$/, '');
  updated = replaceLine(updated, /^\*\*One-liner:\*\*.*$/m, `**One-liner:** ${oneLiner}.`);
  updated = replaceLine(updated, /^\*\*Deliverable:\*\*.*$/m, `**Deliverable:** ${canonical.deliverable}`);
  updated = replaceLine(
    updated,
    /^You will be able to:\s*\*\*.*\*\*.*$/m,
    `You will be able to: **${canonical.objective}**`
  );

  // Output deliverable line inside "Output you will produce"
  updated = replaceLine(updated, /^- \*\*Deliverable:\*\*.*$/m, `- **Deliverable:** ${canonical.deliverable}`);

  // Success criteria block
  const successBlock = [
    `- [ ] ${canonical.deliverable} is completed`,
    '- [ ] Success criteria are written as yes/no checks',
    '- [ ] Next step, owner, and date are defined',
  ].join('\n');
  updated = replaceBlock(updated, '### Success criteria (observable)', '### Output you will produce', successBlock);

  // Guided exercise steps from canonical exercise (split into sentences)
  const steps = toSentenceList(canonical.exercise);
  if (steps.length > 0) {
    const stepLines = steps.map((s, i) => `${i + 1}. ${s.endsWith('.') ? s : `${s}.`}`).join('\n');
    updated = replaceBlock(updated, '### Steps', '### Output format', stepLines);
  }

  // Independent exercise task (simple variation)
  updated = replaceBlock(
    updated,
    '### Task',
    '### Output',
    `Repeat the guided exercise for a different U.S. target account or partner.\n\nUse the same deliverable format: **${canonical.deliverable}**.`
  );

  return updated;
}

function transformContentForUSA(content: string, dayNumber: number) {
  const baseReplacements: Array<[RegExp, string]> = [
    [/\bEurope\b/g, 'USA'],
    [/\bEuropean\b/g, 'U.S.'],
    [/\bEU\b/g, 'U.S.'],
    [/\bEU\/UK\b/gi, 'U.S.'],
    [/\bGDPR\b/g, 'U.S. privacy rules'],
    [/\bVAT\b/g, 'sales tax'],
    [/\bmulti-country\b/gi, 'multi-state'],
    [/\bEU public procurement\b/gi, 'U.S. public procurement'],
    [/\bEU procurement\b/gi, 'U.S. public procurement'],
    [/\bEU tenders\b/gi, 'U.S. public tenders'],
  ];

  const daySpecific: Record<number, Array<[RegExp, string]>> = {
    21: [
      [/\bEU public procurement logic\b/gi, 'U.S. public procurement logic (federal, state, local)'],
      [/\bEU public procurement\b/gi, 'U.S. public procurement'],
    ],
    22: [
      [/\bESPD\b/g, 'SAM registration'],
      [/\beCertis\b/g, 'UEI/registration evidence'],
      [/\beForms\b/gi, 'FAR representations & certifications'],
    ],
    23: [
      [/\bTED\b/g, 'SAM.gov'],
      [/\bCPV\b/g, 'NAICS'],
      [/\bTenders Electronic Daily\b/gi, 'SAM.gov Contract Opportunities'],
    ],
    24: [
      [/\bUK public procurement\b/gi, 'U.S. state & local procurement'],
      [/\bFind a Tender\b/gi, 'state procurement portals'],
      [/\bContracts Finder\b/gi, 'city/county procurement portals'],
    ],
    25: [
      [/\bUSA and MENA procurement entry points\b/gi, 'U.S. federal/state and MENA procurement entry points'],
      [/\bUSA\b/g, 'U.S.'],
    ],
  };

  return applyReplacements(content, [...baseReplacements, ...(daySpecific[dayNumber] || [])]);
}

function main() {
  const europe: CoursePackageV2 = JSON.parse(fs.readFileSync(EUROPE_BASE, 'utf8'));
  const usaCanonical: CanonicalCourse = JSON.parse(fs.readFileSync(USA_CANONICAL, 'utf8'));

  assert(europe.course && europe.lessons, 'Europe base must contain course + lessons.');
  assert(europe.lessons.length === 30, `Europe base should have 30 lessons, found ${europe.lessons.length}`);

  const sourcesByDay = buildLessonSourcesByDay(usaCanonical);

  const canonicalByDay = new Map<number, CanonicalLesson>();
  usaCanonical.lessons.forEach((l) => canonicalByDay.set(l.dayNumber, l));

  const updatedLessons = europe.lessons.map((lesson) => {
    const sources = sourcesByDay.get(lesson.dayNumber);
    assert(sources, `Missing sources for day ${lesson.dayNumber}`);
    const canonical = canonicalByDay.get(lesson.dayNumber);
    const usaTitle = canonical?.canonicalTitle || lesson.title;

    let updatedContent = updateLessonContentSources(lesson.content, sources);
    updatedContent = updateLessonHeaderTitle(updatedContent, usaTitle);
    updatedContent = updateContentWithCanonical(updatedContent, canonical ?? {
      dayNumber: lesson.dayNumber,
      canonicalTitle: usaTitle,
      objective: 'apply the lesson method to a U.S. market scenario',
      keyConcepts: [],
      exercise: 'Apply the method to a relevant U.S. account and document your decisions.',
      deliverable: 'Completed lesson deliverable',
      sources: [],
    });
    updatedContent = transformContentForUSA(updatedContent, lesson.dayNumber);
    const updatedQuestions = buildQuestionsForDay(lesson.dayNumber, lesson.title);

    return {
      ...lesson,
      lessonId: lesson.lessonId.replace('SPORT_SALES_NETWORK_EUROPE_2026_EN', 'SPORT_SALES_NETWORK_USA_2026_EN'),
      title: usaTitle,
  ...
  const outPkg: CoursePackageV2 = {
    ...
    exportedBy: 'codex/refactor-sport-sales-network-usa-from-europe',
    course: {
      ...(europe.course || {}),
      courseId: 'SPORT_SALES_NETWORK_USA_2026_EN',
      ccsId: 'SPORT_SALES_NETWORK_USA_2026',
      name: 'Build a Sport Sales Network in the USA 2026',
      language: 'en',
      isActive: true,
    },
