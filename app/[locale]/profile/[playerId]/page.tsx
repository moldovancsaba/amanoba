                    ...
                    {certificatesLoading ? (
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        {certificatesData.map((cert) => (
                          <div key={cert.certificateId} className="bg-brand-black/20 rounded-lg p-4">
                            <div className="mb-3">
                              <h4 className="text-white font-bold text-lg mb-1">{cert.courseTitle}</h4>
                              {cert.courseName && (
                                <p className="text-gray-400 text-sm">{cert.courseName}</p>
                              )}
                            </div>
                            <div className="mb-3">
                              <div className="text-brand-accent text-2xl font-bold mb-1">
                                {cert.score}%
                              </div>
                              <p className="text-gray-400 text-sm">
                              ...
                              {cert.isRevoked && (
                                <p className="text-red-400 text-sm mt-1">Revoked</p>
                              )}
                            </div>
                            <a
                              href={`/${locale}/certificate/${cert.verificationSlug}`}
                              className="inline-block bg-brand-accent text-brand-black px-4 py-2 rounded-lg font-bold text-sm text-center w-full"
                            >
                              View Certificate
                            </a>
                          </div>
