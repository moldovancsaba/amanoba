            ...
            <>
              <form
                action={async () => {
                  ...
                  const returnTo = resolvedSearchParams.callbackUrl || `/${resolvedParams.locale}/dashboard`;
                  // Redirect to SSO login endpoint
                  redirect(`/api/auth/sso/login?returnTo=${encodeURIComponent(returnTo)}`);
                }}
              >
                <button
                  type="submit"
                  className="w-full bg-brand-accent hover:bg-brand-primary-600 text-brand-white font-semibold text-lg py-4 px-6 rounded-xl transition-colors duration-200 flex items-center justify-center gap-3 shadow-lg hover:shadow-xl"
                >
                  <svg
                    className="w-6 h-6"
                    fill="none"
