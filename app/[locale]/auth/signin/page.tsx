    ...
    <div className="min-h-screen flex items-center justify-center bg-brand-black p-6 sm:p-10">
      <div className="w-full max-w-lg">
        ...
        <div className="bg-brand-white rounded-3xl shadow-2xl p-10 border-2 border-brand-accent">
          ...
          {/* Sign In Form - SSO Login */}
          <form
            action={async () => {
              'use server';
              const resolvedParams = await params;
              const resolvedSearchParams = await searchParams;
              const callbackUrl = resolvedSearchParams.callbackUrl || `/${resolvedParams.locale}/dashboard`;
              
              // Redirect to SSO login endpoint
              const ssoLoginUrl = `/api/auth/sso/login?returnTo=${encodeURIComponent(callbackUrl)}`;
              redirect(ssoLoginUrl);
            }}
          >
            <button
              type="submit"
              className="w-full bg-brand-accent hover:bg-brand-primary-400 text-brand-black font-semibold text-lg py-4 px-6 rounded-xl transition-colors duration-200 flex items-center justify-center gap-3 shadow-lg hover:shadow-xl"
            >
              <Icon icon={MdGpsFixed} size={24} />
