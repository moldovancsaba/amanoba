...
| `docs/product/GDS_ADOPTION_MANIFEST.md` | Local GDS enforcement contract: allowlist/banlist, protected surfaces, required contract paths, and validation commands. | `gds-adoption.json`, `scripts/check-gds-adoption.ts`, `scripts/check-mantine-boundaries.mjs`, UI checks. | Current as of May 25; validates that docs, contract paths, and protected-surface scope remain aligned. |
| `docs/product/GDS_EXCEPTION_REGISTER.md` | Approved local GDS exception scope, user impact, and removal conditions. | `gds-adoption.json`, documented exception file paths, `docs/product/DESIGN_UPDATE.md`. | Current as of May 25; exceptions are explicit and machine-checked. |
