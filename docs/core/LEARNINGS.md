...
**Context**: Same behavior needed in multiple places (e.g. up/down vote on courses, lessons, discussion posts).

**Learning**: Implement the feature once with a **discriminator** (e.g. `targetType`, `targetId`) so one schema, one route, and one UI component serve all contexts. To add a new surface (e.g. vote on discussion posts), extend the discriminator allow-list and reuse the same component; do not add new collections or routes for the same behavior.
...
**Applied In**: Content voting (course, lesson, question, discussion_post); **docs/product/VOTING_AND_REUSE_PATTERN.md**, **docs/architecture/ARCHITECTURE.md** (Core Principles), **docs/architecture/layout_grammar.md** (§9), **docs/core/CONTRIBUTING.md** (Coding Standards).

---

### UI in User Language and Message File Completeness

**Context**: When adding a new locale (e.g. Swahili) or auditing existing ones, some message files still had English strings for key UI (e.g. `courses.title`, `admin.title`), so users selecting that language saw mixed language.

**Learning**: When the user selects a language, the **entire UI** (landing, dashboard, courses, admin, etc.) must be in that language. Each **`messages/<locale>.json`** should contain translated strings for all namespaces used in the app; avoid English placeholders for live locales. For a new locale: (1) add to **`app/lib/i18n/locales.ts`**; (2) create and fully translate **`messages/<locale>.json`**; (3) add to LanguageSwitcher, profile language dropdown, **course-languages.ts**, **email-localization.ts**, **certificate-strings.ts** (if used), API **VALID_LOCALES**, and course-language–keyed UI maps. Periodically audit message files for key keys (e.g. `landing.heroTitle`, `courses.title`, `dashboard.title`, `admin.title`) to catch English leftovers.

**Why It Matters**: Consistent, respectful UX; no confusing mix of languages; single source of truth for “supported language” is the message file content, not just the locale code.

**Applied In**: Swahili (sw) rollout; language file audit (bg, pl, vi, hu, id, pt); **docs/i18n/I18N_SETUP.md**, **docs/product/RELEASE_NOTES.md** v2.9.45.
...
**Applied In**: `docs/core/ENVIRONMENT_SETUP.md`, startup validation script (Phase 2.1).
