...
- ✅ **Reusability**: Searched for existing solutions before creating new code; same feature in 2+ places → one model/API/component with discriminator (see **docs/product/VOTING_AND_REUSE_PATTERN.md**)
...
When the same feature is needed in 2+ places (e.g. voting on courses, lessons, discussion posts), use **one model**, **one API**, and **one UI component** with a discriminator (e.g. `targetType`, `targetId`) to select context. Do not duplicate schemas, routes, or components. To add a new surface, extend the discriminator allow-list and reuse the same component. See **docs/product/VOTING_AND_REUSE_PATTERN.md** and **docs/architecture/layout_grammar.md** (§9).
...
- **docs/core/ENVIRONMENT_SETUP.md** - Setup troubleshooting
