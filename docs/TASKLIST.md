...
| BUG5 | `/admin/players` shows "Premium" type incorrectly | P0 | **Investigate**: Page shows `isPremium` badge (line 213-217), but user types should be GUEST/USER/ADMIN only. Player model has `role: 'user' | 'admin'` and `isAnonymous: boolean`. Replace `isPremium` badge with type logic: GUEST = `isAnonymous === true`, ADMIN = `role === 'admin'`, USER = default. Remove premium filter and stats. | ⏳ PENDING |
