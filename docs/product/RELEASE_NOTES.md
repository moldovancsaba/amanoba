...
- **Course card migration:** Moved the learner-facing course card implementation to `app/components/patterns/gds/CourseCard.tsx` on top of `@sovereignsquad/gds-core/client` `PublicProductCard`, while preserving the existing product call sites through the thin re-export.
...
- **Release notes wiki migration:** Added `npm run release-notes:wiki:export`, documented the GitHub wiki page format/publish workflow, and published the initial Amanoba release-note archive to the wiki.
...
- **Course import API mode switch:** `POST /api/admin/courses/import` now accepts `questionImportMode` with:
  - `add`: add missing questions only (existing questions stay unchanged),
  - `overwrite`: delete existing lesson questions and replace from package,
  - `merge`: backward-compatible default (update matches + create missing).
- **Admin UI controls:** Added question import mode selector in:
  - `app/[locale]/admin/courses/[courseId]/page.tsx`
  - `app/[locale]/admin/courses/page.tsx`
  Mode is sent with import payload; confirmation text and import summary reflect selected behavior.
- **Import stats enriched:** Import API response now includes `questionsDeleted` and `importOptions.questionImportMode` for accurate UI feedback and auditability.
- **Build stabilization fixes:** Resolved build blockers found during `npm run build`:
  - `components/CookieConsentBanner.tsx`: fixed hook order violation (`useEffect` before conditional return),
  - `app/[locale]/admin/achievements/[achievementId]/page.tsx` and `app/[locale]/admin/achievements/new/page.tsx`: expanded criteria type unions to include course-related criteria used by UI,
  - `app/lib/email/email-localization.ts`: changed strict locale maps to partial-locale maps with safe `en` fallback to support expanded locale set.
- **Verification:** `npm run build` passed; `npm run docs:check` passed.

**Rollback:** Revert import-mode changes in the API/UI files and build-stabilization type fixes, then re-run `npm run build` and `npm run docs:check`.
...
- **Archive cleanup:** Normalized stale historical references in high-value archive docs and tasklists to current locations (including cross-repo course-doc paths under `/Users/moldovancsaba/Projects/amanoba_courses/process_them/...`), eliminating broken archive doc tokens.
- **TASKLIST alignment:** Documentation operations item “Clean and normalize high-value historical docs in `docs/_archive/**`” marked done with command evidence; handover package maintenance remains active per cycle.
- **ROADMAP alignment:** Removed items already tracked in TASKLIST from roadmap “future functions” and kept roadmap entries vision-only.
- **Handover refresh:** Updated continuity package docs (`brain_dump`, handoff snapshot, next-window prompt) to carry this new baseline forward.

**Rollback:** Revert archive reference edits and product docs updates from this release section, then re-run `npm run docs:check` to confirm restoration.
...
- **Course-content docs relocation:** Course creation/maintenance docs were moved out of `amanoba/docs` to `/Users/moldovancsaba/Projects/amanoba_courses/process_them` and active references were updated accordingly.
- **Link integrity tooling:** Added `scripts/docs/check-doc-links.mjs` and npm script `docs:links:check`; `docs:check` now validates generated docs plus link integrity in active docs.
- **CI coverage:** Docs workflow updated (`.github/workflows/docs-generated-check.yml`, name: “Docs (generated + links)`) through `npm run docs:check`.
- **Context-transfer package:** Added `docs/handoff/HANDOFF_CONTEXT_WINDOW_2026-02-12.md` and `docs/handoff/NEXT_WINDOW_PROMPT.md`. Updated continuity docs (`docs/core/amanoba_codex_brain_dump.md`, `docs/core/agent_working_loop_canonical_operating_document.md`) for next-window recovery.

**Rollback:** Revert docs folder move commits, restore old `docs/` layout, revert path updates in active docs, remove `scripts/docs/check-doc-links.mjs`, restore previous `docs:check` command and workflow name.
...
- **UI in selected language:** When the user selects a language (e.g. Kiswahili or Russian), all UI elements must be in that language. **`messages/<locale>.json`** is the source for next-intl; each locale file must contain translated strings for the full app (no English placeholders for live locales). **`messages/sw.json`** was completed in Kiswahili accordingly.
- **Language file audit:** Fixed English leftovers in non-English message files: **bg.json** `courses.title` → "Курсове"; **pl.json** `courses.title` → "Kursy", `admin.title` → "Administrator"; **vi.json** `courses.title` → "Khóa học"; **hu.json** `admin.title` → "Rendszergazda"; **id.json** `admin.title` → "Pengelola"; **pt.json** `admin.title` → "Administração". All 12 locale message files now show correct language for key UI (landing, courses, dashboard, admin).
- **Docs:** **docs/i18n/I18N_SETUP.md**, **docs/architecture/ARCHITECTURE.md**, **docs/architecture/layout_grammar.md**, **docs/core/agent_working_loop_canonical_operating_document.md** updated to 12 languages and Swahili (sw). **docs/core/LEARNINGS.md** — new entry "UI in user language and message file completeness".

**Rollback:** Revert Swahili-related commits; remove `sw` from `locales.ts`, `course-languages.ts`, `VALID_LOCALES`, email-localization, certificate-strings, and course UI translation maps; remove or replace `messages/sw.json`. Revert language-audit edits in bg, pl, vi, hu, id, pt message files.
...
- **Policy:** Documented in `docs/features/GEO_IMPROVEMENT_PLAN.md`: do not add in-app links to `/view`; discovery via sitemap, external links, or AI/search only.
...
- **Package format v2:** `/Users/moldovancsaba/Projects/amanoba_courses/process_them/docs/COURSE_PACKAGE_FORMAT.md` defines the schema (packageVersion 2.0, course, lessons, quiz; optional canonicalSpec, courseIdea). Single JSON format for export/import.
...
- **Question import mode (admin option):** Course import now accepts `questionImportMode` so admins can choose question handling when updating a course:
  - `add`: only add missing questions (keep existing question content unchanged),
  - `overwrite`: replace questions for imported lessons with package questions,
  - `merge` (default/backward compatibility): update matched questions and create missing questions.
- **Admin UI:** Course editor and admin course list import now expose a question mode selector (`Questions: Add Only` or `Questions: Overwrite`) and pass mode to the import API.
...
**Reference:** `/Users/moldovancsaba/Projects/amanoba_courses/process_them/docs/COURSE_EXPORT_IMPORT_RECOMMENDATION.md`. Package format is single JSON in UI; API still accepts ZIP for backward compatibility (TASKLIST P2 #6 closed as not planned).
...
### P2 — Multiple courses UI (TASKLIST item 3)

- **Dashboard:** Added a My Courses section that lists **all active enrolments** with per-course progress and a “Next Lesson” action; shows daily learning progress across multiple courses.
- **Course discovery:** `/courses` now shows a **clear Enrol CTA** for courses the learner is not enrolled in and a **Continue Learning** action for enrolled courses.
- **Prerequisites notice:** Course list and course detail show prerequisite notices based on `prerequisiteCourseIds` and completion status; enrol is disabled when enforcement is `hard` and prerequisites are not met (API 403 with `PREREQUISITES_NOT_MET` is handled in UI).
- **APIs:** Course list/detail responses include `prerequisiteCourses` (courseId + name) and `prerequisiteEnforcement` for UI rendering.

### P3 — Course achievements: new types (TASKLIST item 2)

- **Achievement engine:** Added support for new course criteria types **`perfect_week`** (7 consecutive lessons) and **`early_finisher`** (complete within N days). Context now uses course progress timestamps for early-finisher checks.
- **Seeds:** `scripts/seed-achievements.ts` and `scripts/seed-achievements-v2.ts` include **Perfect Week** and **Early Finisher** achievements.
- **Admin UI:** Achievement criteria picker now includes course criteria types, including the new ones.
...
- **Docs**: **docs/i18n/I18N_SETUP.md** updated (§4 Middleware & Routing, §6 Language Switcher & Navigation).
...
- **Docs**: **agent_working_loop_canonical_operating_document.md** — new “Supported Languages and locale” section. **docs/architecture/layout_grammar.md** §8 — supported locales, default by browser, user preference. **docs/i18n/I18N_SETUP.md** — updated to 11 languages, locale detection, and user preference.
...
- **Priority framework:** P1 (highest) → P4 (when capacity allows). All open roadmap-derived work is now in **docs/product/TASKLIST.md** with clear priorities.
...
- **Design doc**: **docs/certification/CERTIFICATE_AB_TEST_DESIGN.md** defines A/B testing for certificate template variants.
...
- **Runbooks**: `/Users/moldovancsaba/Projects/amanoba_courses/process_them/docs/course_runs/SCRUMMASTER_LESZEK_2026_HU__2026-01-31T18-53-33Z.md` plus the paired tasklist entry document the publish/apply workflow, commands, and expected state for each day.
...
- **Environment & docs**: `docs/core/ENVIRONMENT_SETUP.md` now outlines the `EMAIL_PROVIDER` option plus the required env vars per provider, emphasises that the Admin UI placeholder does not switch providers, and notes the root `.npmrc` pins `legacy-peer-deps=true` to keep the NextAuth/Nodemailer peer dependency conflict under control.
...
### Scale plan (/Users/moldovancsaba/Projects/amanoba_courses/process_them/docs/_archive/delivery/2026-01/2026-01-31_MESSY_CONTENT_AUDIT_AND_GRAMMAR_PLAN.md § 7)
...
- **Rephrase rules docs**: `/Users/moldovancsaba/Projects/amanoba_courses/process_them/docs/REPHRASE_RULES_HU.md`, `/Users/moldovancsaba/Projects/amanoba_courses/process_them/docs/REPHRASE_RULES_RU.md`, `/Users/moldovancsaba/Projects/amanoba_courses/process_them/docs/REPHRASE_RULES_PL.md`.
...
- **ROADMAP**: P0 item 1 (Global audit: communication + catalog language integrity) marked **Done**; Tech audit follow-up items 10–12 (P0 Security, P1 Lint/TS, P2 Deprecated/hardcoded) marked **Done**; item 13 (P3) updated to show admin Image and CTA audit done, remainder in `docs/product/P3_KNOWN_ISSUES_BACKLOG.md`.
...
**Documentation**: `/Users/moldovancsaba/Projects/amanoba_courses/process_them/docs/_archive/delivery/2026-01/2026-01-27_RAPID_CHILDREN_COURSES_DELIVERY_PLAN.md` — Post-Delivery Checklist RELEASE_NOTES item and Acceptance Criteria marked complete.
...
- **app/api/auth/anonymous/route.ts**: Comment “Facebook login” → “SSO login”. **docs/sso/SSO_MIGRATION_COMPLETE.md**: Noted data-deletion and auth SSO wording.
...
- **Playbook**: `/Users/moldovancsaba/Projects/amanoba_courses/process_them/docs/reference/QUIZ_QUALITY_PIPELINE_PLAYBOOK.md` updated to document mandatory tiny-loop process for quiz rewriting.
...
- **Public profile schema**: `docs/features/PUBLIC_PROFILE_SCHEMA.md` — list of public fields and section keys.
...
- **AUDIT9 – ARCHITECTURE auth**: `docs/architecture/ARCHITECTURE.md` — auth directory updated: removed `api/auth/facebook/`; added `[...nextauth]`, `anonymous`, `sso/`; Security/Authentication describe SSO and anonymous only.
...
- `/Users/moldovancsaba/Projects/amanoba_courses/process_them/docs/_archive/delivery/2026-01/2026-01-25_QUIZ_QUESTION_CENTRAL_MANAGEMENT_COMPLETE.md`
...
**Documentation**: `/Users/moldovancsaba/Projects/amanoba_courses/process_them/docs/_archive/delivery/2026-01/2026-01-25_QUIZ_QUESTION_CENTRAL_MANAGEMENT_COMPLETE.md`
...
- `docs/certification/CERTIFICATE_CREATION_GUIDE.md` - Complete guide covering:
...
**Documentation**: `/Users/moldovancsaba/Projects/amanoba_courses/process_them/docs/_archive/delivery/2026-01/2026-01-25_GOOGLE_ANALYTICS_CONSENT_MODE_AND_COURSE_PROGRESS_FIX.md`
...
**Documentation**: `/Users/moldovancsaba/Projects/amanoba_courses/process_them/docs/_archive/delivery/2026-01/2026-01-24_COURSE_LANGUAGE_SEPARATION_COMPLETE.md`
...
- `docs/core/ENVIRONMENT_SETUP.md` - Added Stripe environment variables
...
- Created `docs/core/ENVIRONMENT_SETUP.md` with setup guide, troubleshooting, and security best practices (304 lines)
