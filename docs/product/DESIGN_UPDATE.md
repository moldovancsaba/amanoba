...
**Status**: GDS 2.4.3 adopted; Mantine-only runtime active; `@gds/*` packages wired
...
| Course access recovery | `components/patterns/CourseAccessRecoveryActions.tsx` → `@gds/core` `AccessRecoveryPanel` |
...
- **Learner shell** — compose locally from GDS primitives ([#99](https://github.com/sovereignsquad/general-design-system/issues/99)); keep `LearnerPageHeader`.
