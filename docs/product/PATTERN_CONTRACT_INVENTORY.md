...
| Admin shell | `app/[locale]/admin/layout.tsx` | Backlog | Still uses legacy local markup and lucide icons. Must migrate to Mantine `AppShell`, `NavLink`, `Menu`, `Burger`, and `ScrollArea`. |
...
1. `app/[locale]/admin/layout.tsx` still owns a legacy admin navigation shell.
2. `app/[locale]/leaderboards/page.tsx`, `app/[locale]/stats/page.tsx`, and `app/[locale]/onboarding/page.tsx` still contain legacy page-local markup.
3. `app/[locale]/admin/votes/page.tsx` still uses raw table/form markup and lucide icons.
4. Memory-game internals still contain local CSS/layout classes. Game-board internals may remain custom only if framed by Mantine shell/state/card contracts.
5. `AuthShell`, `PublicAppShell`, `AdminAppShell`, `DataToolbar`, and `ResponsiveDataView` are not extracted yet.
...
3. Migrate admin layout to Mantine `AppShell`; then extract `DataToolbar` and `ResponsiveDataView` for admin list pages.
