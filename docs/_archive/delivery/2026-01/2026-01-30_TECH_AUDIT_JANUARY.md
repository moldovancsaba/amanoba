...
- **Lint**: **ESLint** — **0 errors, 0 warnings** (scripts/public ignored; auth, i18n, middleware fixed). **TypeScript**: Enforced in build (`typescript.ignoreBuildErrors: false`); see `docs/_archive/delivery/2026-01/2026-01-28_TYPESCRIPT_AUDIT_COMPLETE.md`.
...
- **Fixes applied**: All prior errors and warnings resolved (Sparkles, unescaped entities, no-img-element, no-unused-vars, anonymous default export, beforeInteractive). `@typescript-eslint/no-unused-vars` uses `argsIgnorePattern`/`varsIgnorePattern`/`caughtErrorsIgnorePattern` `^_`. See `docs/_archive/tasklists/TECH_AUDIT_JANUARY__2026-01-30.md`.
...
Work in this order: **P0 → P1 → P2 → P3**. Track progress in **`docs/_archive/tasklists/TECH_AUDIT_JANUARY__2026-01-30.md`**.
...
**How to use**: Start with P0; then P1 so lint/TS can be re-enabled; then P2 and P3. The tasklist **`docs/_archive/tasklists/TECH_AUDIT_JANUARY__2026-01-30.md`** mirrors this order with checkboxes. TASKLIST and ROADMAP already reference the audit follow-up in the backlog.
